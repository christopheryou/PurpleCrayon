                           ...
                           const std::vector<Point2> &stroke2d)
{
    ...
    // ground plane.
	Point2 front = stroke2d.front();
	Point2 end = stroke2d.back();

	Point3 S, E;
	ScreenPtHitsGround(view_matrix, proj_matrix, front, &S);
	ScreenPtHitsGround(view_matrix, proj_matrix, end, &E);

	Vector3 normalUp = Vector3(0, 1, 0);
	Vector3 normalSurf = ((S - E).Cross(normalUp)).ToUnit();
	float dir = normalSurf.Dot(look);
	if (dir > 0) {
		normalSurf = normalSurf * -1;
	}
	...
	int size = stroke2d.size();
	for (int i = 0; i < size; i++) {
		Point3 point;
		float time;
		Point3 point3D = GfxMath::ScreenToNearPlane(view_matrix, proj_matrix, (stroke2d[i]));
		Ray ray(eye, point3D - eye);
		ray.IntersectPlane(S, normalSurf, &time, &point);
    ...
    for (int i=0; i<ground_mesh_.num_vertices(); i++) {
		...
		Point3 nearPoint = P.ClosestPointOnPlane(S, normalSurf);
		float height = hfunc(normalSurf, curve, nearPoint);
		float distance = P.DistanceToPlane(S, normalSurf);
		if (height != 0) {
			float wd = std::max((float)0, 1 - pow(distance / 5, 2));
			float y = P[1];
			P[1] = (1 - wd) * y + wd * height;
		}
